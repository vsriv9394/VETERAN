function getConvFlux(scheme,UL,UR,gamma,n,Area)
    ...
    if scheme=="Roe"
        ...
        # left flux
        FL      = zeros(size(UL)) # for allocation
        FL[1]   = rL*unL
        ...
        # right flux
        FR      = zeros(size(UR)) # for allocation
        FR[1]   = rR*unR
        ...
        F[4]    = 0.5*(FL[4]+FR[4])-0.5*(l3*du[4]+C1*wi+C2*n[3])
